 ...
 *
 * Supports all Feishu message types: text, post (rich text), image,
 * media (video), file, and audio.
 */
...
import { buildInboundFromFeishuMessage, normalizeFeishuText } from "./inbound-media.js";
...
async function handleIncomingMessage(
  ...
  if (!messageType || !message.content) return;

  // Supported message types
  const supportedTypes = ["text", "post", "image", "media", "file", "audio"];
  if (!supportedTypes.includes(messageType)) return;

  // Parse message into text + attachments
  const inbound = await buildInboundFromFeishuMessage(
    ctx.client,
    message,
    {
      maxInboundImageMb: ctx.account.config.mediaMaxMb,
    },
    { error: (msg) => ctx.log.error(`[feishu:${ctx.account.accountId}] ${msg}`) },
  );
  ...
  if (chatType === "group") {
    ...
    if (!hasAttachment && (!decisionText || !shouldRespondInGroup(decisionText, mentions, ctx.botNames))) {
      return;
    }

    // Keep the cleaned text
    text = cleaned;
  }
  ...
  ctx.log.info(
    `[feishu:${ctx.account.accountId}] Received ${messageType} from ${senderId}: ${text.slice(0, 80)}${attachments.length ? ` (+${attachments.length} attachments)` : ""}`,
  );
  ...
  // Pass attachments to the agent if we have them
  if (attachments.length > 0) {
    inboundCtx.Attachments = attachments;
  }
